...
int DisplayChangeFramePointer(DisplayCtrl *dispPtr,u8* frame_pointer){
	for (int i = 0; i < DISPLAY_NUM_FRAMES; i++)
		{
			dispPtr->framePtr[i] = frame_pointer;
			dispPtr->vdmaConfig.FrameStoreStartAddr[i] = (u32)  frame_pointer;
			XAxiVdma_DmaConfig(dispPtr->vdma, XAXIVDMA_READ, &(dispPtr->vdmaConfig));
			XAxiVdma_DmaSetBufferAddr(dispPtr->vdma, XAXIVDMA_READ, dispPtr->vdmaConfig.FrameStoreStartAddr);
		}
}
